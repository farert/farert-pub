...
def node_next(jct_id):
  nodes = []
  sql = """
select 	id, (select case when calc_km>0 then calc_km else sales_km end from t_jct j left join t_lines l on j.station_id=l.station_id where line_id=?1 and id=?2)-case when calc_km>0 then calc_km else sales_km end as cost
from 	t_jct j left join t_lines l on j.station_id=l.station_id
where 	line_id=?1 
and 	sales_km=(select max(sales_km) 
				  from t_jct j left join t_lines l on j.station_id=l.station_id  
				  where line_id=?1 
				  and sales_km<(select 	sales_km from 	t_jct j left join t_lines l on j.station_id=l.station_id where 	line_id=?1 and 	id=?2))
union
select 	id, case when calc_km>0 then calc_km else sales_km end - (select case when calc_km>0 then calc_km else sales_km end from t_jct j left join t_lines l on j.station_id=l.station_id where line_id=?1 and id=?2) as cost
...
"""
  ...
  cur.execute("select line_id from t_lines l left join t_jct j on j.station_id=l.station_id where id=?", [jct_id])
  for lin in cur:
